  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Bedrock access for all agents
      {
        ...
        Resource = [
          # Allow both the Lambda region and the configured Bedrock region.
          # In practice, some SDK/provider paths may still call the Lambda's region by default.
          "arn:aws:bedrock:${var.aws_region}::foundation-model/*",
          "arn:aws:bedrock:${var.bedrock_region}::foundation-model/*",
          # Some Nova "us." routing / inference profile paths may end up in us-east-2.
          # Allow it explicitly to avoid confusing AccessDenied failures.
...
# Attach AWS-managed SQS execution permissions for Lambda event source mappings
resource "aws_iam_role_policy_attachment" "lambda_agents_sqs" {
  role       = aws_iam_role.lambda_agents_role.name
