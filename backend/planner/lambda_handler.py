...
# Reuse a single event loop across Lambda invocations.
# Using asyncio.run() creates a brand new loop each invocation, which can break
# background async workers (e.g., LiteLLM logging worker) that keep references
# to queues/tasks bound to the previous loop.
...
def _run_async(coro):
    global _LAMBDA_LOOP
    try:
        # If we're already in an event loop, just schedule/await upstream (not expected in Lambda).
    ...
    return _LAMBDA_LOOP.run_until_complete(coro)
...
def lambda_handler(event, context):
    ...
    with observe():
        try:
            ...
            # Run the orchestrator
            _run_async(run_orchestrator(job_id))
